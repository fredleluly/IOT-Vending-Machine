...
# Add new configuration to HardwareConfig class
@dataclass
    ...
    def load_config(self):
        ...
        try:
            ...
            self.hardware_config = HardwareConfig(
                ...
                pulse_per_liter=config['hardware']['pulse_per_liter'],
                ir_sensor_pin=config['hardware']['ir_sensor_pin']
            )
    ...
    def save_config(self):
        ...
        config = {
            ...
            'hardware': {
                ...
                'ir_sensor_pin': self.hardware_config.ir_sensor_pin,
...
class MonitoringWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # Tambahkan timer untuk record quality
        self.record_timer = QTimer()
        self.record_timer.timeout.connect(self.record_quality_data)
        self.record_timer.start(5 * 60 * 1000)  # 5 menit dalam milliseconds
    ...
    def record_quality_data(self):
        """Record quality data every 5 minutes"""
        try:
            if hasattr(self, 'ph_value') and hasattr(self, 'tds_value'):
                ph_text = self.ph_value.text()
                tds_text = self.tds_value.text()
                
                # Skip jika sensor tidak terhubung
                if ph_text == "Not Connected" or tds_text == "Not Connected":
                    return
                    
                # Kirim data ke backend
                success = self.api_client.record_quality({
                    'ph_level': float(ph_text),
                    'tds_level': float(tds_text),
                    'water_level': 0,
                    # 'timestamp': datetime.now().isoformat()
                })
                if not success:
                    logger.warning("Quality data ARE NOT ***=123=***recorded successfully")
                else:
                    logger.info("Quality data recorded successfully")

                
        except Exception as e:
            logger.error(f"Failed to record quality data: {e}")
...
class MachineWidget(QWidget):
    ...
    # Tambahan 2 baris ini
        self.setup_ir_sensor()
        # Start IR detection timer
        self.ir_timer = QTimer()
        self.ir_timer.timeout.connect(self.check_ir_sensor)
        self.ir_timer.start(100)  # Check every 100ms
    ...
    def setup_ir_sensor(self):
        """Initialize IR sensor"""
        if HARDWARE_AVAILABLE:
            try:
                GPIO.setup(self.config.hardware_config.ir_sensor_pin, GPIO.IN)
                logger.info("IR sensor initialized successfully")
            except Exception as e:
                logger.error(f"Failed to initialize IR sensor: {e}")

    def check_ir_sensor(self):
        """Check IR sensor status and update button state"""
        if HARDWARE_AVAILABLE:
            try:
                is_detected = not GPIO.input(self.config.hardware_config.ir_sensor_pin)
                if hasattr(self, 'selected_size'):
                    self.start_button.setEnabled(is_detected and self.selected_size is not None)
                else:
                    self.start_button.setEnabled(False)
            except Exception as e:
            ...
            if hasattr(self, 'selected_size'):
                self.start_button.setEnabled(self.selected_size is not None)
...
class WaterSustainabilityApp(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            cleanup_timer.stop()
               # Final check - force cleanup if needed
            if hasattr(self, 'machine_widget') and \
               hasattr(self.machine_widget, 'water_controller'):
                try:
                    self.machine_widget.water_controller.force_shutdown()
                except:
                    pass
    ...
    def _record_transaction(self, success: bool):
        try:
            if hasattr(self, 'last_transaction'):
                ...
                # Record sale data
                self.api_client.record_sale({
                    'volume': self.last_transaction['size'],
                    'order_id': self.last_transaction['order_id'], 
                    'timestamp': self.last_transaction['timestamp'],
                    'success': success
                })
                
                
                logger.info(f"Transaction and quality data recorded: {transaction_data}")
        except Exception as e:
            logger.error(f"Error recording transaction and quality data: {e}")
    ...
    def force_cleanup(self):
        ...
        # Force water controller shutdown
        if hasattr(self, 'machine_widget') and \
           hasattr(self.machine_widget, 'water_controller'):
            try:
                self.machine_widget.water_controller.force_shutdown()
